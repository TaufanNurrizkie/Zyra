...
export default function ChartSection({ distribution, monthlyData, categoryData, weeklyData, totalPenerima,relawanAktif, persen }) {   // ✅ terima props dari parent
    ...
    const distributionData = distribution.map(item => {
        const key = item.golongan.toLowerCase();
        return {
            name: item.golongan,
            value: item.total,
            color: colors[key],
    ...
    // PERBAIKAN: Mapping data kategori dengan nama yang lebih pendek
    const data = categoryData.map((item, index) => ({
        category: item.jenis_bantuan.length > 15 ?
            item.jenis_bantuan.substring(0, 15) + '...' :
            item.jenis_bantuan,
        fullCategory: item.jenis_bantuan,
        jumlah: item.jumlah,
        fill: COLORS[index % COLORS.length] // Assign warna dari array COLORS
    }));
    ...
    // Custom tooltip untuk kategori chart
    const CategoryTooltip = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
            const data = payload[0].payload;
            return (
                <div className="bg-white p-2 sm:p-3 border border-gray-200 rounded-lg shadow-lg text-xs sm:text-sm max-w-xs">
                    <p className="font-medium text-gray-800 mb-1">{data.fullCategory}</p>
                    <p className="text-xs sm:text-sm" style={{ color: payload[0].color }}>
    ...
    return (
        <div className="mt-4 sm:mt-6 lg:mt-8 space-y-4 sm:space-y-6 lg:space-y-8 px-2 sm:px-4 lg:px-0">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
                ...
                {/* Area Chart - Weekly Trend (Minggu 1-4 bulan ini) */}
                <div ref={(el) => (chartsRef.current[2] = el)} className="bg-white shadow-lg rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 border border-gray-100 hover:shadow-xl transition-all duration-300">
                    <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs sm:text-sm text-gray-500">Penyaluran bantuan bulan ini (Minggu 1-4)</p>
                        </div>
                    </div>
                    ...
                    <div className="w-full overflow-hidden">
                        <ResponsiveContainer width="100%" height={200} className="sm:!h-64">
                            <AreaChart data={weeklyData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                                ...
                                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                                <XAxis dataKey="week" tick={{ fontSize: 10 }} className="sm:text-xs" />
                                <YAxis tick={{ fontSize: 10 }} className="sm:text-xs" />
                                <Tooltip content={<CustomTooltip />} />
                                <Area
                                    type="monotone"
                ...
                {/* Bar Chart - Category Distribution (DIPERBAIKI) */}
